//===-- WDCFrameLowering.cpp - WDC Frame Information --------------------===//
//
...
bool WDCFrameLowering::hasFPImpl(const MachineFunction &MF) const {
  const auto & MFI = MF.getFrameInfo();
  const auto TargetRegisterInfo = Subtarget.getRegisterInfo();
